              ...
              <button
                onClick={async () => {
                  if (window.confirm("Are you sure you want to delete this course permanently?")) {
                    const { deleteCourse } = await import("../../../services/operations/courseDetailsAPI");
                    await deleteCourse({ courseId: course._id }, token);
                    navigate("/admin/dashboard");
                  }
                }}
                className="w-full mt-2 rounded-lg bg-red-800 py-[8px] text-xs font-semibold text-red-100 hover:bg-red-700 transition-all"
              >
                Delete Course (Admin)
              </button>
            )}

...
export default function CourseInformationForm() {
  ...
  const onSubmit = async (data) => {
    ...
    formData.append("tag", JSON.stringify(data.courseTags && data.courseTags.length > 0 ? data.courseTags : ["General"]))
    formData.append("whatYouWillLearn", data.courseBenefits || "Learn key concepts and skills in this course.")
    formData.append("category", data.courseCategory)
    formData.append("status", COURSE_STATUS.PUBLISHED)
    formData.append("instructions", JSON.stringify(data.courseRequirements && data.courseRequirements.length > 0 ? data.courseRequirements : ["Basic computer understanding"]))
    if (data.courseImage) {
      formData.append("thumbnailImage", data.courseImage)
    }
    setLoading(true)
    ...
    >
      ...
      <div className="flex flex-col space-y-2">
        <label className="text-sm text-richblack-5" htmlFor="courseBenefits">
          Benefits / What Students Will Learn <span className="text-xs text-richblack-400">(optional)</span>
        </label>
        <textarea
          id="courseBenefits"
          placeholder="Enter what students will learn in this course (optional)"

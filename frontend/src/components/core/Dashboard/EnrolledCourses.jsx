...
import { getUserEnrolledCourses, unenrollCourse } from "../../../services/operations/profileAPI"
...
export default function EnrolledCourses() {
  ...
  const handleUnenroll = async (courseId) => {
    const success = await unenrollCourse(courseId, token);
    if (success) {
      getEnrolledCourses();
    }
            ...
            enrolledCourses?.map((course, i, arr) => (
              ...
              >
                ...
                <div className="px-3 py-3 flex items-center">
                  {confirmUnenrollId === course._id ? (
                    <div className="flex gap-1">
                      <button
                        onClick={() => handleUnenroll(course._id)}
                        className="rounded-lg bg-red-800 px-3 py-1.5 text-xs font-semibold text-red-100 hover:bg-red-700"
                      >
                        Confirm
                      </button>
                      <button
                        onClick={() => setConfirmUnenrollId(null)}
                        className="rounded-lg bg-richblack-700 px-2 py-1.5 text-xs text-richblack-300 hover:text-richblack-5"
                      >
                        Cancel
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 rounded-lg border border-red-900 bg-red-950/40 px-3 py-1.5 text-xs text-red-300 hover:bg-red-900/60 transition-all"
                      title="Unenroll from Course"
                    >
                      <MdExitToApp size={15} /> Unenroll
                    </button>
                  )}
                </div>
              </div>

    ...
    return (
        <nav className={`z-[10] flex h-14 w-full items-center justify-center border-b-[1px] border-b-richblack-700 text-white translate-y-0 transition-all ${showNavbar} `}>
            ...
            <div className='flex w-11/12 max-w-maxContent items-center justify-between '>
                ...
                {/* Login/SignUp/Dashboard & Theme Toggle */}
                <div className='flex gap-x-4 items-center'>
                    {/* Theme Toggle Button */}
                    <button
                        onClick={() => dispatch(toggleTheme())}
                        title={`Switch to ${theme === 'dark' ? 'Light' : 'Dark'} Mode`}
                        className='rounded-full border border-richblack-700 bg-richblack-800 p-2.5 text-richblack-100 hover:text-yellow-50 hover:bg-richblack-700 transition-all cursor-pointer flex items-center justify-center'
                    >
                        {theme === 'dark' ? (
                            <FiSun className='text-lg text-yellow-50' />
                        ) : (
                            <FiMoon className='text-lg text-richblack-25' />
                        )}
                    </button>
